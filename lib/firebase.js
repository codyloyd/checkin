    ...
    teamRef.once('value').then(data => {
      ...
      if (userId) {
        this.fetchUserData(userId).then(data => {
          teamRef.update({members: members.filter(member => member !== data.val().displayName)})
        })
      } else if (displayName) {
        teamRef.update({members: members.filter(member => member !== displayName)})
      }
    })
  },
    ...
    return Promise.all(promises).then(promises => {
      return promises.reduce((obj, team, i) => {
        if (team) {
          ...
          this.removeTeamFromUser({userId: auth.currentUser().uid, teamId: teams[i]})
        }
        return obj
      }, {})
    })
